...
$header_title = 'Добавить запись';

include_once( TEMPLATEPATH . '/templates/header.php' );

?>

	<div class="add-form-env">
		<div class="form-header">
			<h3>Добавить запись</h3>
			<a href="/admin/" class="link">Назад</a>
		</div>
		<form action="/admin/add" method="post">
			...
			<div class="field-group">
				<select name="parent_id">
					<option value="0">Выберите родительскую запись</option>
					<option value="0">Корневой</option>
					
					<?php if( $data['all_objects'] ): ?>
					
						<?php foreach( $data['all_objects'] as $object ): ?>
				...
				<?php if( isset( $data['response'] ) ): ?>
					...
					<?php else: ?>
						...
						<div class="form-response failed">
							...
							<span>При добавлении записи возникла ошибка</span>
						</div>
